...
export class IOSBackendService implements OnModuleInit {
  ...
  // Token cache for exchanged Cognito → Munawar JWT tokens
   ...
   * 
   * Cognito JWT (from iOS) → Extract email → Login to Munawar's backend → Get Munawar's JWT
   */
  private async exchangeCognitoToken(cognitoJwt: string): Promise<string | null> {
    try {
      console.log('🔄 [IOSBackend] Exchanging Cognito JWT for backend JWT...');
      
      // 1. Decode Cognito JWT to get user email (no verification needed - just extract payload)
      const parts = cognitoJwt.split('.');
      if (parts.length !== 3) {
        console.error('❌ [IOSBackend] Invalid JWT format');
        return null;
      }
      ...
      if (!email) {
        console.error('❌ [IOSBackend] No email found in Cognito JWT');
        return null;
      }
      
      console.log(`🔄 [IOSBackend] Cognito JWT email: ${email}`);
      
      // 2. Check cache for existing Munawar's JWT for this user
      const cacheKey = `jwt_${email}`;
      const cached = this.tokenCache.get(cacheKey);
      if (cached && cached.expiry > new Date()) {
        console.log(`✅ [IOSBackend] Using cached backend JWT for ${email}`);
        return cached.token;
      }
      ...
      if (!agentPassword) {
        console.error('❌ [IOSBackend] IOS_BACKEND_PASSWORD not configured - cannot exchange token');
        return null;
      }
      ...
      if (!agentEmail) {
        console.error('❌ [IOSBackend] IOS_BACKEND_EMAIL not configured - cannot exchange token');
        return null;
      }
      
      console.log(`📤 [IOSBackend] POST /api/agent/auth/login (as agent: ${agentEmail})`);
      ...
      if (!response.ok) {
        const error = await response.text();
        console.error(`❌ [IOSBackend] Token exchange login failed: ${error}`);
        return null;
      }
      
      const data = await response.json();
      if (!data.success || !data.token) {
        console.error('❌ [IOSBackend] Token exchange did not return a token');
        return null;
      }
      ...
      console.log(`✅ [IOSBackend] Token exchange successful! Cached for ${email}`);
      return munawarJwt;
      
    } catch (error: any) {
      console.error(`❌ [IOSBackend] Token exchange error:`, error.message);
      return null;
    }
  }
  ...
  private async getEffectiveJwt(userJwt?: string): Promise<string> {
    ...
    if (userJwt) {
      console.log('🔄 [IOSBackend] Received JWT from iOS, attempting exchange...');
      ...
      console.warn('⚠️ [IOSBackend] Token exchange failed, trying original token...');
      return userJwt;
    }
