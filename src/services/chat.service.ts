...
export class ChatService {
  ...
  private sessionStorage = new Map<string, {
    history: any[],
    profile: any,
    lastActivity: Date
  }>();
   ...
   */
  async processMessage(message: string, sessionId: string, selectedLLM: string = 'gpt-4o'): Promise<ChatResponseDto> {
    try {
      ...
      const psychProfile = await this.parallelLLM.analyze(message, historyBeforeCurrent, sessionId, selectedLLM);
      ...
      const analysis = this.personalityAnalyzer.analyzePersonality(
        message,
        sessionId,
        history
      );
   ...
   */
  private buildEnhancedInsightsFromProfile(
    profile: any,
    insights: any
  ): string {
    const parts: string[] = [];
  ...
  private buildCleanProfile(profile: any) {
    ...
    const getScore = (value: any, highThreshold: number = 4, lowThreshold: number = 2): 'high' | 'medium' | 'low' | null => {
    ...
    try {
      ...
      if (bf && dass) {
        ...
        if (neuroticism === 'high' && anxiety === 'low' && bf.Confidence >= 0.7 && dass.Confidence >= 0.7) {
      ...
      if (bf || mbti || ennea) {
        ...
        if (mbti) {
          const ei = mbti.EI;
          if (ei === 'E' && mbti.Confidence && (typeof mbti.Confidence === 'object' ? mbti.Confidence.EI >= 0.7 : mbti.Confidence >= 0.7)) {
            extraversionSignals.push('high');
          } else if (ei === 'I' && mbti.Confidence && (typeof mbti.Confidence === 'object' ? mbti.Confidence.EI >= 0.7 : mbti.Confidence >= 0.7)) {
        ...
        if (extraversionSignals.length >= 2 && highCount > 0 && lowCount > 0) {
          conflicts.push(`Mixed Extraversion/Introversion signals (${highCount} high, ${lowCount} low)`);
        }
      }
      ...
      if (rse && (bf || dass)) {
        ...
        if (isLowSelfEsteem && rse.Confidence >= 0.7) {
          ...
          if (inconsistent) {
            conflicts.push('Low Self-Esteem (RSE) conflicts with low Neuroticism/Depression indicators');
          }
        }
      }
      ...
      if (bf && dass) {
        ...
        if (consci === 'high' && stress === 'high' && bf.Confidence >= 0.7 && dass.Confidence >= 0.7) {
          conflicts.push('High Conscientiousness + High Stress may indicate perfectionism-related tension');
        }
      }
    ...
    try {
      ...
      if (conflicts.some(c => c.includes('Extraversion'))) {
        const signals: Array<{ source: string; value: 'high' | 'low' | 'medium' | null; conf: number }> = [];
        if (bf) signals.push({ source: 'Big5', value: getScore(bf.Extraversion ?? bf.extraversion), conf: bf.Confidence ?? 0.8 });
        if (mbti) {
          const eiValue: 'high' | 'low' | null = mbti.EI === 'E' ? 'high' : (mbti.EI === 'I' ? 'low' : null);
          signals.push({ source: 'MBTI', value: eiValue, conf: typeof mbti.Confidence === 'object' ? (mbti.Confidence as any).EI : (mbti.Confidence ?? 0.8) });
        }
        if (ennea) {
          const enneaValue: 'high' | 'low' | null = ennea.Primary_Type === 7 ? 'high' : (ennea.Primary_Type === 5 ? 'low' : null);
      ...
      if (bf && (mbti || ennea) && !conflicts.some(c => c.includes('Extraversion'))) {
        ...
        if (agreementCount >= 2) {
          ...
          integrations.similarities.push({
            models: ['Big5 Extraversion', mbti ? 'MBTI' : '', ennea ? 'Enneagram' : ''].filter(Boolean),
      ...
      if (rse && (bf || dass) && !conflicts.some(c => c.includes('Self-Esteem'))) {
        ...
        if (isLowSelfEsteem) {
          ...
          if ((neuroticism === 'high') || (depression && depression !== 'low')) {
            ...
            integrations.similarities.push({
              models: ['RSE', neuroticism === 'high' ? 'Big5 Neuroticism' : '', depression ? 'DASS Depression' : ''].filter(Boolean),
      ...
      if (bf) {
        ...
        if (neuroticism !== null && neuroticism >= 2.5 && neuroticism <= 3.5) {
          ...
          bf.integrationNote += (bf.integrationNote ? '; ' : '') + 'Moderate/balanced emotional sensitivity';
      ...
      if (bf) {
        ...
        if (extraversion !== null && extraversion >= 2.5 && extraversion <= 3.5) {
          if (!bf.integrationNote) bf.integrationNote = '';
          bf.integrationNote += (bf.integrationNote ? '; ' : '') + 'Balanced social energy (ambivert)';
          integrations.mildCases.push({
            trait: 'Extraversion',
      ...
      if (dass) {
        ...
        if (anxiety !== null && depression !== null && stress !== null) {
          ...
          if (allMild && (anxiety >= 1 || depression >= 1 || stress >= 1)) {
            dass.integrationNote = 'All emotional distress levels are mild/normal - generally stable emotional state';
            integrations.mildCases.push({
              trait: 'DASS (all dimensions)',
              values: { anxiety: anxiety.toFixed(1), depression: depression.toFixed(1), stress: stress.toFixed(1) },
    ...
    if (integrations.similarities.length > 0 || integrations.mildCases.length > 0 || integrations.conflictResolutions.length > 0) {
  ...
  private buildEnhancedInsights(
    ...
    if (psychProfile) {
      ...
      // Self-esteem
      if (psychProfile.selfEsteem.level === 'low' && psychProfile.selfEsteem.indicators.length > 0) {
        psychInsights.push(`Self-Esteem: Low (${psychProfile.selfEsteem.indicators[0]})`);
      }

      // Core needs (SDT)
      if (psychProfile.coreNeeds.analysis && !psychProfile.coreNeeds.analysis.includes('reasonably met')) {
