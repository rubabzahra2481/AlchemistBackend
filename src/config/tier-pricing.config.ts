...
const VALID_TIERS: UserTier[] = ['free', 'basic', 'standard', 'pro', 'elite'];

/**
 * For local testing: TIER_OVERRIDE=basic|standard|pro|elite in .env
 * makes the backend treat the current user as that tier (ignores Munawar's tier).
 * Unset or set to "free" to use normal tier from API.
 */
export function getTierOverride(): UserTier | null {
  const v = process.env.TIER_OVERRIDE?.toLowerCase().trim();
  if (!v || v === 'free') return null;
  if (VALID_TIERS.includes(v as UserTier)) return v as UserTier;
  return null;
}
