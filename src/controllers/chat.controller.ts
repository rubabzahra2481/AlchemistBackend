...
export class ChatController {
  ...
  async getTierInfo(
    ...
    // Include credit status so iOS can block when user is over tier credits (one API for all)
    const creditCheck = await this.creditService.checkCredits(userId, userTier);
    const creditStats = await this.creditService.getUsageStats(userId, userTier);
    ...
    return {
      ...
      credits: {
        ...
        tokensUsed: creditStats.tokensUsed,
        tokensIncluded: creditStats.tokensIncluded,
        creditsUsed: creditStats.creditsUsed,
        creditsIncluded: creditStats.creditsIncluded,
        baseAllowance: creditStats.baseAllowance,
        topUpCarried: creditStats.topUpCarried,
        topUpAddedThisMonth: creditStats.topUpAddedThisMonth,
        usagePercentage: creditStats.usagePercentage,
  ...
  @Get('credits')
  @ApiOperation({ summary: 'Get credit usage statistics (anonymous)' })
  ...
  @Get('credits/:userId')
  @ApiOperation({ summary: 'Get credit status for a user (for iOS: use to block when over)' })
  @ApiResponse({ status: 200, description: 'Returns allowed, tokensUsed, tokensIncluded, message. If allowed is false, app should block sending.' })
  async getCreditsForUser(@Param('userId') userId: string) {
    ...
    const flow = {
      ...
      stages: [
        ...
        },
        {
          ...
          input: [
            'System prompt with Decision Intelligence Expert personality',
